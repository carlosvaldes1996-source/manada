...
> | **Status** | 🟢 Fase de diseño cerrada por Carlos (2026-07-10). Implementación por bloques en chats separados: un bloque, una revisión, un commit. **F1–F3 implementados y aprobados (en disco, pendientes de commit); F4 refinado y convergido con Carlos el 2026-07-10 tras usar el flujo real** (ver §1.5). |
...
### 🔹 Bloque F4 — Recomendación consultiva "El plan de {mascota}" (resuelve O3) · diseño convergido 2026-07-10
...
- **Modificar (candidatos):** `comenzar/recomendacion/recommendation-view.tsx`; `lib/recommend.ts` (exponer **top-N** para alternativas, no solo top-1); reutilizar la anticipación (`AnticipationCapsule`/`lib/anticipation`) para la **propuesta de reposición**.
- **Dependencias:**
  - **F3** (calidad del plan) ✅.
  - **O5 / catálogo real (EN PAUSA):** las alternativas y la puerta de marca necesitan **productos reales por perfil** (con `variantId`). Mientras la recomendación corra sobre datos demo, el diseño se documenta pero la implementación plena (y el flujo *"Agregar → carrito"*) **espera la reconexión al backend**. La recomendación principal puede maquetarse contra el estado local.
  - **Pet Experience:** el *"recuerdo ambiental"* (dashboard/perfil/PDP *"recomendado para Toby"*) y *"el plan vive en la mascota"* cruzan con `current_food`/perfil (Pet Experience B3/B6). Coordinar, no duplicar.
  - **F5:** las salidas y *"Salir no descarta el journey"* se alinean con el momento de cuenta/tarjeta, incl. la **hipótesis futura de tarjeta-en-archivo** (§1.5).
