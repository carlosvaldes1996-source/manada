...
- **Dependencias:**
  - **F3** (calidad del plan) ✅.
  - **⏳ O5 / catálogo real — PENDIENTE (pasada de integración SEPARADA, decisión D32):** F4 corre sobre catálogo **demo** (`lib/data/catalog`); los productos demo **no tienen `variantId`**, así que "Sumar al pedido" enruta pero **no** deposita el ítem en el carrito real (**artefacto conocido y validado**, no bug de UX; ver el guard de `cart-provider.tsx`). La reconexión de `lib/recommend` al catálogo real de Medusa (Store API, con `variantId` real → alternativas/puerta de marca/Add-to-cart de punta a punta) es el **siguiente trabajo** y va en su propio bloque/commit. **Un chat futuro no debe asumir que O5 ya está reconectado.**
  - **Pet Experience:** el *"recuerdo ambiental"* (dashboard/perfil/PDP *"recomendado para Toby"*) y *"el plan vive en la mascota"* cruzan con `current_food`/perfil (Pet Experience B3/B6). **F4 NO escribe `currentFoodId` ni construye el recuerdo ambiental** (coordinar, no duplicar); el plan se re-deriva del perfil en cada visita.
