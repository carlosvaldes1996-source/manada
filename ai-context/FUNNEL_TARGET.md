...
> **🔄 Evolución de la ejecución (D44, 2026-07-12) — la tesis no cambia, sí su presentación.** Tras recorrer el flujo real, la card resultó demasiado alta y "explicaba" de más. Principio rector de Product: *el onboarding ya convenció → la pantalla decide, no persuade.* La ejecución pasó a una **carta de plan**: en desktop **2 columnas** ("decidir" | "el valor") que **convierten altura en ancho** (CTA + anticipación sin scroll); **razones bajo demanda** (disclosure "¿Por qué esta?"); **stats en 1 línea**; **anticipación comprimida a ~2 líneas** con su **lugar reservado** intacto (ahí crece la suscripción); **riel de complementos eliminado** (no era parte de este diseño objetivo); y **"ya come otra marca" como buscador inteligente** (autofocus + filtrado en vivo + exploración) que **rearma y GUARDA** el plan sin empujar el cambio de marca. Las capas (a)–(f) siguen existiendo; cambia cómo se distribuyen (varias pasan a on-demand).
...
- **🔄 Rediseño de UI/UX (D44, 2026-07-12 · commit `d274925`) — reemplazo total del layout, sin tocar backend ni la tesis de §1.5.** El smoke reveló que la implementación D32/D33 quedó **demasiado alta** (~2 pantallas hasta el CTA) y con aire de PDP/landing. Se rediseñó `recommendation-view.tsx` como **carta de plan**: (1) **desktop en 2 columnas** ("decidir" | "el valor") que convierte altura en ancho; (2) **razones on-demand** (disclosure "¿Por qué esta?") — el onboarding ya convenció; (3) **stats de 2 tiles → 1 línea** + ración deduplicada; (4) **anticipación comprimida a ~2 líneas** (días de aviso en Popover) conservando su lugar reservado a la suscripción (límite D29 intacto); (5) **fuera el riel de complementos** (lo más "ecommerce"; no estaba en §1.5); (6) **"ya come otra marca" = decisión de primer nivel** con **Sheet buscador inteligente** (autofocus + filtrado en vivo + exploración) que rearma el plan en modo `owned` y lo **GUARDA** (`assignFood`, sin carrito). **Sin cambios de backend:** reusa `listProducts`/`addItem`/`assignFood`→`current_food_id`; **F5/muro de registro intacto** (invitado sigue a `/crear-cuenta`). `tsc`+`eslint`+`build` verdes; **smoke visual en vivo pendiente**. Detalle y rationale: `DECISIONS.md` D44.

- **🔄 Reconciliación de UI + suscripción real + responsive (D62, 2026-07-31) — supersede la presentación de D44, no su tesis §1.5.** La rama `cristobal-cambios` tenía un diseño **validado** de esta pantalla (columna angosta mobile-first, cierre con check de éxito + **ración como prueba**, **tarjeta destacada** con banda de color + badges de formato/duración, **alternativas en línea**), pero su HEAD (2026-07-20) es **anterior** a la suscripción real (D55–D58): su tarjeta se construyó sobre el **recordatorio proxy de D29**. Copiarla tal cual habría revertido el moat de suscripción y el manejo de estado → se **reconcilió** (validado con Carlos): se adopta el **diseño de Cristóbal** sobre la **funcionalidad de `main`**. `recommendation-view.tsx` pasa a **una sola columna** (`max-w-2xl`, arregla el overflow móvil del layout de 2 columnas), con la **suscripción real dentro** (`PlanManadaCard` + compra única, patrón de la PDP / D56) y **alternativas en línea** (`AltCard`; se retira `AlternativesSheet`). Además se **conservan** las mejoras propias: **aviso de sobrepeso** (`overweightSignal`), **funnel unificado** (`addToOrder`→`/carrito`; el muro invitado→`/crear-cuenta` **ya no existe** — cierre de facto de la hipótesis F5 §1.6 hacia "sin muro") y **guard de hidratación** (`isHydrating`). Sin tocar recomendador/cálculos RER-MER/contratos/APIs/estado (`foodPlan` solo para mostrar ración y "Dura X días"). `tsc`+`eslint` limpios; **smoke Playwright 16/16** (perro sano · móvil 390px · sobrepeso). Detalle: `DECISIONS.md` D62.
