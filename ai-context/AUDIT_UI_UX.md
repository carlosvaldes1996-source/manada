...
| U009 | **Aplicar `font-variation-settings` (opsz) a Fraunces.** Se carga el eje óptico y nunca se usa: titulares sin el refinamiento por el que se paga. | 3 | 2 | **P1** | Pendiente · Polish (3.4) | `globals.css`, `layout.tsx` |
| U010 | **Resolver el solape de escala tipográfica** `h1` (28–40) vs `display-l` (33–48): producen tamaños ambiguos según viewport. | 3 | 2 | **P1** | Pendiente · Polish (3.4) | `globals.css`, `ui/section-heading.tsx` |
...
| U028 | **Contraste AA en texto chico** (`text-muted` #7A7064 / notas 12–13px) sobre blanco/`neutral-50`. Verificar y ajustar. | 3 | 1 | **P1** | Pendiente · verificar en Polish (3.4) | `globals.css` |
| U029 | **Verificar contraste del botón miel en hover** (`miel-600` + texto `neutral-800`). | 2 | 1 | **P2** | Pendiente · verificar en Polish (3.4) | `globals.css`, `ui/button.tsx` |
...
| ID | Descripción | Imp | Esf | Prioridad | Estado | Archivos afectados |
|---|---|---|---|---|---|---|
| U040 | **Coherencia de fechas de anticipación.** Home: "se acaba el 2 jul / ~5 días". PDP/checkout: "llega mañana 28 jun / hoy 27 jun". Sistema: hoy 28 jun. Para una marca cuyo core es anticipar, una fecha mal calculada destruye la promesa. | 5 | 2 | **P0** | Pendiente | `lib/anticipation.ts`, `lib/demo-data.ts`, `pet/anticipation-capsule.tsx`, `commerce/honest-shipping-block.tsx` |
...
| U043 | **PLP no debe ocultar catálogo por defecto.** El filtro "Para Toby" activo de entrada puede esconder lo que el usuario busca. Mostrar todo + sugerir personalización. | 4 | 2 | **P0** | Pendiente | `app/(tienda)/categoria/[slug]/page.tsx`, `commerce/filters-panel.tsx`, `commerce/personalization-banner.tsx` |
| U044 | **Subir specs de valor en PDP** (ración 145 g/día, duración ~100 días, $/kg) a un módulo destacado arriba, no enterrados en tab "Descripción". | 4 | 2 | **P1** | Pendiente | `app/(tienda)/producto/[slug]/page.tsx`, `pet/feeding-schedule.tsx` |
| U045 | **Jerarquizar PDP hacia la suscripción** (modelo recurrente = margen). Hoy "Agregar" y "◍ Suscribir" pesan igual. | 5 | 2 | **P1** | Pendiente | `app/(tienda)/producto/[slug]/page.tsx`, `commerce/subscription-box.tsx` |
| U046 | **Reaseguro "pausa/cancela cuando quieras" junto al toggle en PDP** (primer punto de decisión), no solo en checkout. | 4 | 1 | **P1** | Pendiente | `commerce/subscription-box.tsx` |
| U047 | **Reformular los "4 pasos" del checkout de una pantalla.** La numeración promete secuencia inexistente y sugiere "falta mucho". | 4 | 2 | **P1** | Pendiente | `commerce/checkout-stepper.tsx`, `app/checkout/page.tsx` |
| U048 | **Aclarar alcance del toggle de suscripción en checkout** (¿todo el pedido? ¿solo el alimento?) para evitar miedo a recurrencia no deseada → abandono. | 4 | 2 | **P1** | Pendiente | `app/checkout/page.tsx`, `commerce/subscription-box.tsx` |
| U049 | **Hacer visible la creación del "perfil de recompra"** (hoy en nota chica al final del checkout). Decisión importante mal señalizada. | 3 | 1 | **P1** | Pendiente | `app/checkout/page.tsx`, `commerce/order-summary.tsx` |
| U050 | **Celebrar visualmente el ahorro por suscripción** ("−$8.249") en carrito/checkout; hoy tiene el mismo peso que el resto. | 4 | 2 | **P1** | Pendiente | `commerce/order-summary.tsx` |
| U051 | **Distinguir línea suscripción vs. compra única** en el carrito más allá de un badge chico (agrupar "se repite" / "una vez"). | 3 | 2 | **P1** | Pendiente | `commerce/cart-item.tsx`, `app/(tienda)/carrito/page.tsx` |
| U052 | **Reducir densidad del cross-sell** (home + PDP + carrito a la vez). Definir reglas de cuándo aparece para no saturar. | 4 | 2 | **P1** | Pendiente | `commerce/product-rail.tsx`, `app/page.tsx`, `app/(tienda)/producto/[slug]/page.tsx`, `app/(tienda)/carrito/page.tsx` |
| U053 | **Reducir la densidad del nombre "Toby".** ~5 veces solo en la home: cruza de cálido a uncanny. Alternar con "tu perro"/"él". | 4 | 2 | **P1** | Pendiente | `lib/demo-data.ts`, `app/page.tsx`, copy global |
| U054 | **Barra de completitud del perfil accionable:** listar los 2 campos que faltan para 100%, no solo el porcentaje. | 3 | 2 | **P1** | Pendiente | `pet/pet-profile-header.tsx`, `pet/pet-edit-card.tsx`, `app/(tienda)/cuenta/mascotas` |
| U055 | **Cuadrar conteo y contenido en PLP** ("24 productos" con 8 cargados; "mejores para Toby" sin explicar criterio). | 3 | 2 | **P1** | Pendiente | `app/(tienda)/categoria/[slug]/page.tsx`, `commerce/product-grid.tsx` |
| U056 | **Hacer coincidir barra y número** de la cápsula ("≈18% restante" vs. barra casi vacía). | 3 | 1 | **P1** | Pendiente | `pet/anticipation-capsule.tsx`, `ui/progress.tsx` |
| U057 | **Renombrar "Mi recompra"** (jerga) y unificar con "Mascotas"/"Mis mascotas" (hoy 3 nombres para zonas que se confunden). | 3 | 1 | **P1** | Pendiente | `config/nav.ts`, `layout/navbar.tsx`, `layout/bottom-nav.tsx`, `layout/footer.tsx` |
...
| U059 | **Quitar "Buscar" como ítem del bottom-nav** (redundante con el search del header) y dar al search móvil un acceso visible. | 3 | 2 | **P2** | Pendiente | `layout/bottom-nav.tsx`, `layout/header.tsx`, `ui/search-bar.tsx` |
| U060 | **Search disponible en móvil** (<768px hoy desaparece por completo sin alternativa inmediata). | 3 | 2 | **P2** | Pendiente | `layout/header.tsx`, `ui/search-bar.tsx` |
| U061 | **Nivelar la IA del nav:** mezcla categorías (Alimento/Accesorios/Farmacia) con destinos personales (Mi recompra) en el mismo nivel. | 3 | 3 | **P2** | Pendiente | `config/nav.ts`, `layout/navbar.tsx`, `layout/mega-menu.tsx` |
| U062 | **Coherencia breadcrumb ↔ nav:** breadcrumb usa "Perro" como nivel 1 pero el nav no tiene eje Perro/Gato. | 2 | 2 | **P2** | Pendiente | `ui/breadcrumb.tsx`, `config/nav.ts` |
| U063 | **Separar filtro personal de filtros de catálogo** (el chip "Para Toby ✕" se mezcla con filtros normales y confunde qué se filtra). | 3 | 2 | **P2** | Pendiente | `commerce/filters-panel.tsx`, `commerce/personalization-banner.tsx` |
| U064 | **Explicar el orden "Recomendado para Toby"** (criterio caja negra) con tooltip/nota. | 2 | 1 | **P2** | Pendiente | `commerce/product-grid.tsx`, `ui/tooltip.tsx` |
| U065 | **Arreglar enlaces placeholder del perfil** ("Mi suscripción" enlaza a sí misma; varios `#`). | 3 | 2 | **P2** | Pendiente | `app/(tienda)/cuenta/**`, `config/nav.ts` |
| U066 | **Redondeo de precios de suscripción** ($46.741 con decimales raros genera fricción en CLP). Definir política de redondeo. | 3 | 1 | **P2** | Pendiente | `lib/format.ts`, `lib/demo-data.ts` |
| U067 | **Decidir descubrimiento en PLP:** "Cargar más" vs. scroll infinito vs. paginación con conteo claro. | 3 | 2 | **P2** | Pendiente | `ui/pagination.tsx`, `commerce/product-grid.tsx` |
...
| U069 | **Aislar la cápsula de anticipación** (el momento más valioso compite con accesos rápidos pegados debajo). Darle aire/jerarquía. | 3 | 2 | **P2** | Pendiente | `app/page.tsx`, `pet/anticipation-capsule.tsx` |
| U070 | **Lógica de cercanía en métodos de despacho** ("Retiro en Providencia" aparece con perfil en Ñuñoa). | 2 | 2 | **P3** | Pendiente | `commerce/shipping-method.tsx`, `app/checkout/page.tsx` |
| U071 | **Selector de direcciones guardadas** en checkout (hoy inputs sueltos sin gestión). | 3 | 3 | **P2** | Pendiente | `app/checkout/page.tsx`, `ui/field.tsx` |
| U072 | **Eliminar redundancia "¿Ya desparasitaste a Toby?"** repetida en home, PDP y perfil. | 2 | 1 | **P3** | Pendiente | `pet/recommendation-card.tsx`, `app/page.tsx`, `app/(tienda)/producto/[slug]/page.tsx` |
...
| U085 | **Animación de la barra de progreso de comida** (llenado) para reforzar la "anticipación". | 3 | 2 | **P2** | Pendiente | `pet/anticipation-capsule.tsx`, `ui/progress.tsx`, `lib/motion.ts` |
...
| U087 | **Reconsiderar el `pulse` de la cápsula:** anima una alerta de comida que se acaba → puede generar ansiedad en vez de cuidado tranquilo. | 3 | 1 | **P1** | Pendiente | `pet/anticipation-capsule.tsx`, `lib/motion.ts` |
| U088 | **Afordancia visual de botón** en "¿Por qué te lo decimos?" (transparencia bien pensada, mal señalizada). | 2 | 1 | **P2** | Pendiente | `pet/anticipation-capsule.tsx`, `ui/button.tsx` |
| U089 | **Transiciones de ritmo entre secciones** (el bloque pino oscuro aparece de golpe entre secciones claras). | 2 | 2 | **P2** | Pendiente | `app/page.tsx`, `ui/section.tsx` |
...
| U101 | **Reparar el ritmo vertical** (`.section` 40/72px anulado con `padding-top:0` + overrides inline). | 2 | 2 | **P2** | Pendiente | `app/page.tsx`, `ui/section.tsx`, `globals.css` |
