...
- **Packshots normalizados (D52):** todo packshot que llega de Medusa se **normaliza server-side** (`GET /api/packshot`, `sharp`: aplana sobre blanco → recorta el borde → re-encuadra a un cuadrado con margen uniforme, producto ~88 %) y `ProductImage` está graduado a **`next/image`** con loader custom (`srcset` responsive, WebP cacheado, cero CLS). Resultado: asset con **fondo blanco o transparente** se ve **igual de grande y consistente** sin editar cada imagen; los pozos de media pasan a **blanco**. Verificado punta a punta contra el dev server. Doc: `COMPONENT_LIBRARY` (ProductImage) · `DESIGN_SYSTEM §6`. Owner del código: `lib/media/packshot.ts` + `app/api/packshot/route.ts`.
