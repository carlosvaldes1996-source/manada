...
## Siguiente paso
**▶️ Fase 4 — Arquitectura técnica (D19/D20).** Estructura física ✅ (D20). Orden de trabajo restante (detalle en `ROADMAP.md`):
1. **Validar stack backend:** Medusa.js vs alternativas (Vendure/Saleor/custom Next + Postgres/Supabase), con las suscripciones custom y el Perfil de Mascota como criterios rectores. El elegido vivirá en `apps/backend` (regla 2 de `ARCHITECTURE.md §2`).
2. **Proveedores Chile:** pagos (Webpay/MercadoPago/Khipu), courier, boleta SII, WhatsApp Business API.
3. **Modelo de datos** (`DATABASE.md`, hoy en borrador) y **contratos API** (`API.md`). Al aprobarse el primer contrato compartido en `API.md`, recién ahí nace `packages/shared` (D20).
