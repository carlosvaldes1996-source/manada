...
| D82 | 🟢 Cuentas — **la compra de invitado se ADOPTA, no se duplica**. Carlos vio dos clientes con el mismo correo en el Admin (uno `Guest`, uno `Registered`): no era un bug de Manada sino diseño de Medusa —el índice es `UNIQUE (email, has_account)`, no `UNIQUE (email)`, y `validateCustomerAccountCreation` no lanza cuando ya hay un invitado—, pero el daño real no era la fila de más: **la orden pagada, la suscripción, las mascotas y la tarjeta seguían colgando de un `customer_id` que la cuenta nueva no veía**. Adoptar = marcar `has_account` sobre la fila que YA existe → las **12 tablas** que referencian `customer_id` quedan intactas; **fusionar queda prohibido** (`flow_customer` tiene UNIQUE por cliente, `customer_account_holder` PK compuesta, y son datos de pago). La adopción exige **demostrar posesión del correo**: la identidad nace ligada pero con clave aleatoria —vínculo INERTE— y la consuma `POST /store/account/confirm`, autenticado. **Una sola lógica central** sirve al job de D65 y al registro manual. Se acepta la enumeración de correos, encapsulada. 10 casos de integración 11/11 verde, tras reparar el arnés que estaba roto de antes. **NO en vivo:** `AUTO_ACCOUNT_ENABLED` sigue en `false` hasta el E2E |
| D84 | 🟢 Despacho — **la cobertura deja de ser un supuesto: solo Región Metropolitana, con candado en el servidor**. Entró una compra real desde Valparaíso y la operación (despacho manual) solo cubre Santiago: ese pedido nació imposible de cumplir. La regla se instala donde ya vivía la otra mitad del despacho (`lib/shipping.ts`, dueño único del *cuánto* → ahora también del *dónde*), sin módulo ni tabla nuevos. **El candado NO está en el formulario**: bloquear el `<Select>` habría dejado pasar un carrito preparado por fuera, así que la validación corre en las **dos únicas puertas al cobro** —`flow-payment` y `subscription-payment`— **antes de crear nada en Flow**, de modo que un pedido fuera de zona no llega a existir ni como intento de pago. Tolerante al leer (acepta `RM`, `Región Metropolitana`, `Santiago`: una dirección vieja no puede costar una venta cumplible) y estricto al decidir (sin región ⇒ no pasa). El front **avisa, no decide**, y degrada al revés que D81: **sin `coverage` no bloquea a nadie**, porque una cobertura inventada en el cliente costaría ventas buenas mientras que un falso "sí" lo ataja el servidor ⇒ **el backend se despliega primero**. Se cierra también el origen: `/cuenta/direcciones` deja de guardar región como texto libre. **NO está en vivo** |
