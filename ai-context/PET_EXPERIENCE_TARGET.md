...
4. **Alimento↔mascota:** ✅ persistido vía `assignFood`→PATCH al agregar al carrito (D34-i4). El refuerzo por `line_item.metadata.pet_id` + subscriber `order.placed` (re-anclar la fecha a la COMPRA, no al carrito) queda como afinamiento posterior.
