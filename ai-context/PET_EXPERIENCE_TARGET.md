...
### 🔹 Bloque 5 — Editar campos del perfil (Dialog real) ✅ COMPLETADO (2026-07-11, D37)
Elimina los toasts muertos ("se conecta en la siguiente fase").

> **✅ Implementado y verificado** (2026-07-11, D37 — se adelantó a B4 por decisión de Carlos: la persistencia ya existía [D34], no dependía de la foto). **Creado** `components/pet/pet-edit-dialog.tsx`: un solo Dialog para peso (Input numérico; editarlo a mano marca `weightSource:"exacto"` → cierra el hedge "estimado" de F3), raza (**reutiliza `BreedCombobox`** de F2), esterilización (RadioGroup Sí/No) y salud (texto separado por comas → `conditions[]`). **Semántica setter-only:** solo se envía lo que cambió; campo vacío no borra (excepto salud: limpiar el texto limpia la lista). **`updatePet` nuevo en el `PetProvider`** (no existía; B4 lo preveía pero no bloqueaba): optimista en memoria + recálculo de `completeness` + PATCH `/store/pets/:id` con sesión (mismo patrón que `assignFood`). **Muertos eliminados:** el `notify()` de `mascotas-view` ("la edición se conecta en la siguiente fase") en peso/raza/esterilización/salud y el botón "Editar perfil" del hero — todos abren el mismo Dialog ("un solo lugar de edición"); "Alimento" sigue ruteando a la tienda (§1.3). **Patrón React:** el formulario vive en un componente interno montado al abrir (Radix desmonta al cerrar) → borrador fresco por apertura sin `setState`-en-efecto (regla del compiler). **Verificado:** `tsc`+`eslint`+`build` verdes; `/cuenta/mascotas` → 200; PATCH E2E con el body exacto del dialog → 200 y persistido (peso 11.5 exacto · Beagle · esterilizado · `['renal','sobrepeso']`), `conditions:[]` limpia, 500 kg → 400 del validador. Validación UI manual = Carlos.
...
1. ✅ **Módulo `pet`** en Medusa (D34, 2026-07-11): tabla `pet` con `customer_id` indexado (campo plano, no link — rationale en `DATABASE.md §8`), enums a nivel DB, `avatar_url` y `current_food_id`/`food_assigned_at`. Endpoints `GET/POST /store/pets` + `PATCH /store/pets/:id` (autenticados, propiedad por token, zod en el borde, 404 en ajenas). Contrato en `API.md §9`.
2. ✅ **Hidratación real** (D34-i4): `PetProvider` hidrata al autenticarse y **empuja las mascotas de invitado** al backend (espejo de `transferCart`); `addPet` persiste con sesión; `assignFood` = optimista + PATCH (el backend estampa `food_assigned_at`, reloj único). El `localStorage` andamio del B4 ya no será necesario para los datos (solo la foto usará blob/preview local). **El espejo del INVITADO caduca a las 24 h (D77):** `manada.guest_pets` no tenía TTL y una mascota creada sin cuenta quedaba en el dispositivo indefinidamente, porque las dos rutas de limpieza (autenticarse / cerrar sesión) exigen una transición de sesión que un anónimo nunca cruza. El sello es **de creación y no se refresca al escribir** — si se refrescara, la propia hidratación (que reescribe el espejo en cada carga) renovaría la ventana para siempre. Los snapshots sin `savedAt` se tratan como vencidos.
3. **Foto:** upload a blob store + persistir `avatar_url` (columna y PATCH ya existen).
4. ✅ **Alimento↔mascota:** persistido vía `assignFood`→PATCH al agregar al carrito (D34-i4) **+ re-anclaje a la compra (D35, 2026-07-11):** subscriber `order.placed` re-estampa `food_assigned_at` a la fecha de la orden cuando la orden incluye el alimento asignado. **Sin `line_item.metadata.pet_id`** (decisión D35: habría duplicado el vínculo; el match es `order.items.product_id` ↔ `pet.current_food_id`, fuente única).
5. ✅ **Editar** (D37, 2026-07-11): `PetEditDialog` consume el PATCH con validación server-side; `UpdateMyPetInput` ampliado (peso/fuente/raza/esterilización/condiciones, setter-only).
...
> - **O5 → ✅ RESUELTO (2026-07-10, D33):** la recomendación corre sobre el **catálogo real** (server component + `listProducts()`; `lib/recommend` es lógica pura). Con `variantId` real, "Sumar al pedido" **sí** llena el carrito real — el síntoma del carrito vacío desapareció. Además, al sumar se llama `assignFood()` (seam B6) → enciende la anticipación real.
