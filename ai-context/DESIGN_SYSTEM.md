...
|---|---|---|---|---|---|---|---|---|---|---|
| `#FFFFFF` | `#FAF6F0` | `#F0EAE1` | `#E0D8CC` | `#C4BAAB` | `#9E9486` | `#6F665A` | `#5A5249` | `#403A33` | `#2A2722` | `#1A1814` |

> **Ajuste Polish 3.4 (U028):** `neutral.500` pasó de `#7A7064` a `#6F665A` para que `text.muted` cumpla **AA 4.5:1** también sobre Arena (5.2:1) y `neutral.100` (4.7:1); sobre blanco queda 5.6:1. Con el valor anterior fallaba sobre `neutral.100` (4.06:1).
...
|---|---|---|---|---|---|
| `display-xl` | Fraunces | 600 | 44 → 60 | 1.05 | Hero principal. |
| `display-l` | Fraunces | 600 | 36 → 48 | 1.08 | Hero secundario, portadas. |
| `h1` | Fraunces | 600 | 28 → 36 | 1.15 | Título de página. |
...
> **Ajuste Polish 3.4 (U010):** rangos re-escalonados **sin solape entre niveles a ningún viewport** (h1 28–36 < display-l 36–48 < display-xl 44–60; antes h1 llegaba a 40 y display-l partía en 33, produciendo jerarquías ambiguas según el ancho).

> **Eje óptico de Fraunces (U009):** la fuente se carga como variable con `opsz` 9–144 y cada nivel fija su corte: `display-xl` 144 · `display-l` 120 · `h1` 72 · `h2` 48 · `h3` 32 (corte editorial de alto contraste en los héroes) · el **nombre de la mascota** usa `opsz` 18 (corte de texto, más suave y cálido).
...
- **Anticipación proactiva:** la cápsula "A {nombre} le quedan ~5 días" entra sola con slide+fade suave y su **barra de comida se llena con calma** (0 → nivel real, 900 ms ease-out). *(Polish 3.4 · U085/U087: reemplaza al pulso en Miel original — un destello tipo alerta sobre "se acaba la comida" transmitía urgencia, no cuidado tranquilo.)*
...
bg.brand-soft, radius.xl, padding 24
  ...
  → la cápsula entra con slide+fade y la barra se llena con calma (motion §10)
```
