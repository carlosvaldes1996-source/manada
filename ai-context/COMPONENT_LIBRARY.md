...
| **SubscriptionProvider** / `useSubscriptions` | **Estado global de las suscripciones del cliente (D56·C).** Hidrata `GET /store/subscriptions` con la sesión (patrón `PetProvider`; fetch inline en el efecto por la regla `react-hooks`); expone `subscriptions` / `isLoading` / `refresh` / `activeForProduct(productId)`. Lo consumen la **Home** (la `PetStatusCard` pasa a "centro del plan" si hay plan activo para el alimento de la mascota activa —match `product_id === currentFoodId`—: badge "Plan activo", próximo envío, 📦 en la timeline, "Ver mi plan") y **`/cuenta`** (`subscriptions-list`). La Home llama `refresh` al montar (refleja un plan recién creado). Mutaciones de gestión → Bloque D. |
| **PlanManageSheet** (+ `PlanManageProvider` / `usePlanManage`) | **Gestión del plan (D56·D)** dentro del `Dialog` existente: cambiar frecuencia · saltar próximo envío · pausar/reanudar · cancelar (confirmación inline). Lee la suscripción **viva** del `SubscriptionProvider` por id y re-hidrata tras cada mutación (`updateMySubscription` → `PATCH /store/subscriptions/:id`). **Un solo punto de entrada** (`usePlanManage().open(sub)`, patrón `SubscribeFlow`): la MISMA Sheet se abre desde la **Home** (acción "Gestionar plan" de la `PetStatusCard`) y desde **`/cuenta`** ("Gestionar" por fila). Copy **factual** (ninguna acción promete entrega automática; no hay scheduler aún). |
