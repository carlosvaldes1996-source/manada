...
| Componente | Cuándo usar |
|---|---|
| **ProductImage** | Packshot de producto — el ÚNICO punto que conoce la dualidad de `Product.imageUrl` (D36): URL real del Admin → **`next/image`** (`fill` + `object-contain`, `sizes` responsive); emoji placeholder (U090) → span decorativo. **El contenedor padre debe ser `relative`** (lo exige `fill`) y define el aspect-ratio (cero CLS). El encuadre uniforme lo da el **normalizador** (`/api/packshot`, `jimp`: aplana sobre blanco + recorta + margen ~88 %), **no** un padding por card. **Pipeline de render (D66):** el `src` es `packshotSrc()` → `/api/packshot` (imagen **base** JPEG q90) y el **optimizer nativo de Next** hace el `srcset` + **AVIF/WebP** (ya **no** hay loader custom; ~−81 % bytes). Prop **`priority`** (eager + preload) para candidatas a LCP —la 1ª fila del listado la marca `ProductGrid` (`priorityCount`)—. Pozos de media en **`bg-white`**. Lo usan ProductCard, CartItem, PDP, recomendación, checkout y food-selector; **ningún sitio debe renderizar `imageUrl` directo**. `packshotSrc` y constantes: `lib/media/packshot.ts`. |
...
| **ShippingPolicyNote** | **La política de despacho real, en PDP y carrito (D81).** Muestra las dos ramas en dos líneas: el beneficio de la suscripción arriba (verde, es el que decide la compra) y el piso de la compra única abajo. Consume `getShippingPolicy()` (backend = fuente única de los montos) y `lib/shipping-copy.ts` (dueño único del texto). Degrada a una sola rama si `subscriptionFreeShipping` es `false`. No promete fecha ni comuna: el despacho es manual y aún no se calcula por comuna. |
| **FreeShippingBar** | Estado del envío gratis en carrito/drawer. Dos modos: con `includedBySubscription` confirma que el plan ya incluye el despacho **y oculta la barra de progreso** (empujar hacia un umbral que ya no aplica sería presionar por un beneficio que el comprador ya tiene); si no, progreso hacia el umbral con celebración en verde al alcanzarlo. |
| **DeliveryEstimateBlock** | Estimación de entrega (fecha + costo reales por comuna). **Sin uso en producción**: falta el cálculo por comuna, así que las pantallas montan `ShippingPolicyNote`. Vive en el styleguide hasta que ese cálculo exista. *(Antes `HonestShippingBlock`, renombrado en D81.)* |
...
| **SubscriptionProvider** / `useSubscriptions` | **Estado global de las suscripciones del cliente (D56·C).** Hidrata `GET /store/subscriptions` con la sesión (patrón `PetProvider`; fetch inline en el efecto por la regla `react-hooks`); expone `subscriptions` / `isLoading` / `refresh` / `activeForProduct(productId)` / **`subscriptionForProduct(productId)`** (D57·R1 — vigente = activo **o** pausado, no cancelado). Lo consumen la **Home** (la `PetStatusCard` pasa a "centro del plan" si hay plan para el alimento de la mascota activa —match `product_id === currentFoodId`—) y **`/cuenta`** (`subscriptions-list`). La Home llama `refresh` al montar (refleja un plan recién creado). **D57·R1:** un plan **pausado** cuenta como plan (`subscriptionForProduct`) — la Home invita a **reanudar** (abre la Sheet), no a la PDP. Mutaciones de gestión → Bloque D. |
| **PlanManageSheet** (+ `PlanManageProvider` / `usePlanManage`) | **Gestión del plan (D56·D)** dentro del `Dialog` existente: cambiar frecuencia · saltar próximo envío · pausar/reanudar · cancelar (confirmación inline). Lee la suscripción **viva** del `SubscriptionProvider` por id y re-hidrata tras cada mutación (`updateMySubscription` → `PATCH /store/subscriptions/:id`). **Un solo punto de entrada** (`usePlanManage().open(sub)`, patrón `SubscribeFlow`): la MISMA Sheet se abre desde la **Home** (acción "Gestionar plan"/"Reanudar plan" de la `PetStatusCard`) y desde **`/cuenta`** ("Gestionar" por fila). Copy **factual** (ninguna acción promete entrega automática; no hay scheduler aún). **D57:** confirmación inline (`ConfirmInline`) también antes de **Pausar** y **Saltar** (R2, además de Cancelar); **reanudar** muestra un **momento de éxito in-sheet** (reusa el `Dialog` + `framer-motion`, "Tu plan sigue en marcha") en vez del toast plano (R3). |
...
| **PetStatusCard** | Protagonista de la Home logueada (D42): retrato (columna desktop / cuadrado inline móvil), estado escaneable ("Le quedan ~11 días"), línea de tiempo del saco (Compra→Hoy→Se acaba), UNA acción dominante y franja persistente "Plan de {nombre}". Degrada honesto sin alimento/peso. **Evoluciona por estado de suscripción (D56·C / D57·R1·R4):** **activo** → **"Tarjeta miembro" premium** (superficie **Pino** + acentos **Oro** vía el scope `[data-premium]`, CTA dorado —variante `subscribe`—, nombre de la mascota en Oro, 📦 en la timeline, badge "Ahorras $X", "Gestionar plan"); **pausado** → card claro, badge "Plan pausado", CTA único "Reanudar plan" (abre la `PlanManageSheet`), la fila solo comunica estado; **sin plan** → nudge sutil "Actívalo →" a la PDP; **sin suscripción/alimento** → "Pedir de nuevo · $" → carrito. |
