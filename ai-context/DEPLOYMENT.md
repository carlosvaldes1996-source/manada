...
**Pago con Flow (D58 · backend, Railway):** en Railway `FLOW_API_KEY`, `FLOW_SECRET_KEY` (credenciales del comercio, del panel de Flow — **nunca commitear**) y `FLOW_API_URL` (**`https://www.flow.cl/api`** en producción; `https://sandbox.flow.cl/api` en pruebas). Los callbacks los arma el backend con `MEDUSA_BACKEND_URL` (webhook `urlConfirmation`=`/flow/confirmation`, `urlReturn`=`/flow/return`) y `STOREFRONT_URL` (redirect final a `/checkout/confirmacion`) — **ambas ya deben estar seteadas** (§2). El frontend **no** recibe llaves de Flow. Sin `FLOW_API_KEY`/`FLOW_SECRET_KEY`, el endpoint de pago falla claro ("Faltan FLOW_API_KEY…"). **Importante:** en el panel de Flow, la cuenta debe tener habilitado el ambiente correspondiente; el `urlConfirmation` debe ser una **URL pública** alcanzable por Flow (en prod es `api.tumanada.cl`; en local, ngrok). Plantilla local: `apps/backend/.env.template`.
...
3. **Variables del backend** (§2, tabla): `NODE_ENV=production`, `DATABASE_URL=${{Postgres.DATABASE_URL}}`, `REDIS_URL=${{Redis.REDIS_URL}}`, `JWT_SECRET`/`COOKIE_SECRET` (aleatorios), `STORE_CORS`/`ADMIN_CORS`/`AUTH_CORS`, `MEDUSA_BACKEND_URL`, `STOREFRONT_URL`, `RESEND_API_KEY`/`RESEND_FROM`, y **pago Flow (D58)** `FLOW_API_KEY`/`FLOW_SECRET_KEY`/`FLOW_API_URL=https://www.flow.cl/api`. Si aún no hay dominio, usar primero la URL `*.up.railway.app` en `MEDUSA_BACKEND_URL`/CORS y ajustar al conectar `tumanada.cl` (el `urlConfirmation` de Flow se deriva de `MEDUSA_BACKEND_URL`, así que debe ser público).
...
- [ ] **Pago con Flow (D58):** `FLOW_API_KEY`/`FLOW_SECRET_KEY`/`FLOW_API_URL=https://www.flow.cl/api` en Railway + E2E de una compra real (crea pago → paga en Flow → webhook confirma → orden creada + correo) — ⬜ implementado, pendiente llaves+deploy+smoke.
