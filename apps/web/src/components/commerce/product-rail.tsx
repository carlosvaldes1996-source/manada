 ...
 */
export function ProductRail({ title, overline, products, href, linkLabel, className }: ProductRailProps) {
  return (
    <section className={cn("flex flex-col gap-4", className)}>
      <SectionHeading overline={overline} title={title} href={href} linkLabel={linkLabel} as="h3" />
      <ul className="-mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain px-4 pb-2 [scrollbar-width:none] md:mx-0 md:px-0">
        {products.map((p) => (
